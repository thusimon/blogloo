...
import { LOCALE, LOCALE_COUNTRY_MAPPING } from '../types';
...
const ArticleInfo = ({articles, listId}: ArtileInfoPropsType) => {
  ...
  const updateLocale = (idx: number) => {
    ...
    updateSideListIdx(listId);
  };

  console.log(35, state.listId);
  const updateSideListIdx = (listId: string) => {
    dispatch({type: Actions.UpdateArticleListId, data: {listId}});
  }

  return <div className={`article-info-container ${listId === state.listId ? 'article-info-container-selected' : ''}`}>
    <div className='article-info-basic' onClick={() => updateSideListIdx(listId)}>
    ...
    <div className='article-info-locale-select'>
      <DropDown selectCallBack={updateLocale}>
        <img src={usFlag} style={imageListClass} title='English' data-se='en'></img>
        <img src={twFlag} style={imageListClass} title='中文正体' data-se='zh_TW'></img>
        <img src={cnFlag} style={imageListClass} title='中文简体' data-se='zh_CN'></img>
      </DropDown>
    </div>
  </div>
}
